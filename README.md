...
### Keeping Everything Up-To-Date (Beginner friendly)

There are TWO kinds of updates you will do:

- Repo updates: new files, fixes, and features we add to this project
- Package updates: newer versions of apps (from `nixpkgs`) and tools like Home Manager

Follow this exactly. Replace `<host>` with your host name (e.g., `laptop`, `desktop`, `vm@personal`) and `<user>` with your username (e.g., `casper`).

1) Update this repository (get latest changes)
```bash
cd ~/MyNixOS
git pull
```
What this does: downloads the latest version of this project. No system changes happen yet.

2) Optional: Update packages to newer versions
```bash
# Update ALL inputs (nixpkgs, home-manager, etc.) pinned in flake.lock
nix flake update

# OR update a single input only (safer when testing)
nix flake lock --update-input nixpkgs
```
What this does: refreshes the lock file (`flake.lock`) so your system uses newer packages. You can review the change with:
```bash
git diff flake.lock
```

3) Apply the updated configuration to your system
```bash
sudo nixos-rebuild switch --flake .#<host>
```
This will build and activate the new system. First run can take a while.

4) Optional: Update your user’s Home Manager config
```bash
home-manager switch --flake .#<user>
```

5) If something goes wrong, roll back safely
```bash
# Roll back system to previous working generation
sudo nixos-rebuild switch --rollback

# Roll back Home Manager
home-manager rollback

# Revert package updates only (undo lockfile change)
git restore --source=HEAD~1 flake.lock   # if you committed flake.lock
git checkout -- flake.lock               # if you haven’t committed yet
```

6) Free disk space after many updates (optional)
```bash
sudo nix-collect-garbage -d
sudo nix-store --optimise
```
...
Notes and tips:
- You don’t have to run step 2 every time. If you only did a `git pull`, you are just applying repo changes (no package bumps).
- After `nix flake update`, always commit the new `flake.lock` so you can reproduce the same versions later:
  ```bash
  git add flake.lock && git commit -m "chore: update flake inputs"
  ```
- To see what hosts exist, run `nix flake show`.
- Our MSSQL Docker container updates when you rebuild or when Docker pulls a newer image. To force a pull manually:
  ```bash
